...
**时间复杂度**与排序即`O(n log n)`相同。 由于我们使用的是标准排序函数。 但这确实更快，因为内置排序函数使用[内省排序](https://www.geeksforgeeks.org/c-qsort-vs-c-sort/)。
...
时间复杂度与排序相同，即`O(n log n)`。由于我们使用的是标准排序函数。但这确实更快，因为内置排序函数使用了内省排序。
