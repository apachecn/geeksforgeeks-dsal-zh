...
一个简单的解决方案是将链表的内容复制到数组，然后在数组中找到最长的回文子数组，但是此解决方案是不允许的，因为它需要额外的空间。

该思想基于[链表的迭代式反转过程](https://www.geeksforgeeks.org/write-a-function-to-reverse-the-nodes-of-a-linked-list/)。 我们遍历给定的链表，并从左开始逐个反向地翻转链表的每个前缀。 反转前缀后，我们找到从反转前缀开始的最长公共列表，并在反转前缀之后找到列表。
