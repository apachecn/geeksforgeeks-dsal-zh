...
即，如果当前节点是`A`，并且其左子节点是`B`（`A —>> B`），则将创建具有键`A`的新克隆节点（例如`cA`），并将其放置为 `A —>> cA ->> B`（`B`可以是`NULL`或非`NULL`左子项）。 将正确设置右子指针，即，如果对于当前节点`A`，右子在原始树中是`C`（`A —>> C`），则对应的克隆节点`cA`和`cC`将像`cA --->> cC`。

![Binary_Tree(1)](img/dd107df84ab9ba61c70f318c43edbacb.png)

**2\.** 根据原始树在克隆树中设置随机指针，即，如果节点`A`的随机指针指向节点`B`，则在克隆树中，`cA`将指向`cB`（`cA`和`cB`是树中的新节点） 与原始树中的节点`A`和`B`对应的克隆树）。
