...
**方法**：创建一个递归函数，例如`reverse(head, k)`。 该函数接收每组`k`节点的头或第一个节点。 通过应用[系列 2](https://www.geeksforgeeks.org/reverse-doubly-linked-list-set-2/) 中讨论的方法，可以反转一组`k`个节点。反转一组`k`个节点后，该函数检查列表中是否存在下一组节点。 如果组存在，则使用下一个组的第一个节点对其自身进行递归调用，并使用该组的下一个和上一个链接进行必要的调整。 最后，它返回反转组的新头节点。
