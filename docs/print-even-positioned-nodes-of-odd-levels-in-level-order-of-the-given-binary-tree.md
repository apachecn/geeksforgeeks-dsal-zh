# 按照给定二叉树的级别顺序打印奇数级别的偶数定位节点

> 原文:[https://www . geeksforgeeks . org/print-奇数层节点-给定二叉树的一级顺序/](https://www.geeksforgeeks.org/print-even-positioned-nodes-of-odd-levels-in-level-order-of-the-given-binary-tree/)

给定一棵二叉树，任务是在树的级别顺序遍历中打印奇数级别的偶数位置的节点。根被认为是级别 **0** ，任何级别的最左边的节点被认为是位置 **0** 的节点。
**例:**
...
**方法:**要逐层打印节点，请使用层级顺序遍历。思路基于[逐行打印级次遍历](https://www.geeksforgeeks.org/print-level-order-traversal-line-line/)。为此，逐级遍历节点，并在每一级后切换奇数级标志。同样，将每一级中的第一个节点标记为偶数位置，并在每次处理下一个节点后切换它。
以下是上述方法的实施:
