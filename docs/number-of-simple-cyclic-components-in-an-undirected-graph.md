...
因此，为了解决此问题，我们首先确定断开连接图的所有[连通组件。 为此，我们使用深度优先搜索算法。 为了使 DFS 算法起作用，需要维护一个“找到的”数组，以记述递归函数 DFS 发现的所有顶点。 一旦发现了特定连通组件的所有元素（例如 vertices（9，2，15，12）形成了一个连接图组件），我们将检查该组件中的所有顶点的度数是否等于 2。 如果是，则增加计数器变量“ count”，该变量表示在给定图中找到的单循环组件的数量。 为了说明我们目前正在处理的组件，我们也可以使用向量数组“ curr_graph”。](https://www.geeksforgeeks.org/connected-components-in-an-undirected-graph/)
