...
给定一个仅由小写字母组成的字符串`str`和一个数组`arr[][]`，该数组代表给定字符串`str`上的范围查询，其中每个查询包含 3 个整数，`{L, R, N}`，这样对于每个查询，我们都必须输出在查询中指定的给定范围`[L, R]`中的第`N`个最小字符。
...
> **说明**：
>
> 第一个范围`[2, 4]`或子字符串`"fbc"`中的最小字符是`b`。
>
> 第二个范围`[1, 6]`或子字符串`"afbccd"`中的最小字符是`c`。
>
> 第三个范围`[1, 8]`或整个字符串的最小字符是`e`。
> 
...
> **说明**：
>
> 第一个范围`[1, 4]`或子字符串`"geek"`中的最小字符是`e`。
>
> 第二个范围`[3, 7]`或子字符串`"eksfo"`中的最小字符是`k`。
>
> 第三个范围`[4, 13]`或子字符串`"sforgeeks"`中的最小字符是`g`。

**朴素的方法**：朴素的方法是运行从`L`到`R`的循环，并生成此范围内的子字符串。 找到子字符串后，[对子字符串排序](https://www.geeksforgeeks.org/sort-string-characters/)，以在排序的字符串中找到第`N`个字符。
...
*   由于我们为每个查询对子字符串排序，因此，总体时间复杂度为`O(N ^ 2 * Log(N))`。

**高效方法**：想法是利用二维[哈希表](https://www.geeksforgeeks.org/hashtable-in-java/)。 哈希表`H[][]`初始化为`N`行和 **26 列**，其中`N`表示字符串的长度，而 26 表示总长度 26 个小写字母。

哈希表的原理是，对于每个索引`i`，其中`i`的范围为 1 到`N`，我们跟踪第`i`个索引或之前的 26 个小写字母中每个字母的出现次数。 为此，使用[散列](http://www.geeksforgeeks.org/hashing-data-structure/)，其中将每个字符按以下方式视为数字：
...
因此，对于每个查询`{L, R, N}`，我们通过从`H[R][j]`，其中`j`以[哈希表](https://www.geeksforgeeks.org/hashtable-in-java/)的形式介于 0 到 25 之间。 此后，我们要做的就是从 0 遍历到 25 遍历，并添加结果数组的内容。 每当总和等于或超过`N`时，代表该索引的字符就是所需的答案。

例如，如果在第 5 个索引`[j = 4]`上，总和超过`N`，则答案为`e`，因为 4 等于字符`e`。 下面是示例案例的图形表示：

![](img/b5167dcf834b7bea30e2e41d0ab463e9.png)

该图像描述了与给定字符串相对应的哈希表。 对于范围`[2, 4]`，我们从`H[4]`减去`H[2 - 1 - 1]`的内容。 我们得到：

![](img/52529395c1412d9ead7d5f5d7b0ee352.png)
