...
> **输入**：`initList = 15 <=> 16 <=> 6 <=> 7 <=> 17`
>
> **输出**：`finalList = 15 <=> 7 <=> 17`
> 
> **输入**：`initList = 5 <=> 3 <=> 6 <=> 8 <=> 4 <=> 1 <=> 2 <=> 9`
>
> **输出**：`finalList = 5 <=> 3 <=> 1 <=> 9`

这个想法是一个遍历双链表的节点，并得到具有数据的节点的指针。 请按照[本帖子](https://www.geeksforgeeks.org/delete-a-node-in-a-doubly-linked-list/)中使用的方法删除那些节点。
