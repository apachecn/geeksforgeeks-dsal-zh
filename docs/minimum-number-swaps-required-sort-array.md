...
我们仍然可以通过使用哈希映射来提高复杂性。 这里的主要操作是循环内部的`indexOf`方法，这使我们付出了`n * n`的代价。 通过使用哈希映射存储索引，我们可以将此部分改进为`O(n)`。 仍然使用排序方法，因此复杂度无法提高到`O(n Log n)`。
...
1.  如果根据排序后的数组，`i`不在正确的位置，则：

2.  我们将使用我们构建的哈希表中的正确元素填充该位置。 我们知道应该在此处的正确元素是`temp[i]`，因此我们从哈希映射中查找该元素的索引。

3.  交换所需元素后，我们将哈希映射的内容相应地更新，如`temp[i]`到第`i`个位置，以及`arr[i]`到`temp[i]`的较早位置。
