...
**方法**：可以使用[散列](http://www.geeksforgeeks.org/hashing-data-structure/)的概念解决问题。 在此问题中要观察的重点是，此处的字符位置无关紧要，因此我们将计算字符的出现频率，如果所有字符都相同，那么我们就完成了操作，无需删除任何字符来使字符频率相同，否则我们可以逐个遍历所有字符，并将它们的频率降低一，如果所有频率都相同，那么我们可以认为，可以通过最多去除一个字符使字符频率相同，并且如果频率不匹配，那么我们将再次增加该频率并循环其他字符。
