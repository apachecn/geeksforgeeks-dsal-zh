...
给定`N`个整数的数组`Arr`，它们代表图形的节点。 在[按位与](https://www.geeksforgeeks.org/bitwise-operators-in-c-cpp/)不等于`0`的那些对之间定义边。 任务是查找图中是否存在长度为 3 的循环。
...
`bi[0][1]`，`bi[2][1]`和`bi[4][1]`等于 1。这意味着以下对`(Arr[0], Arr[2])`，`(Arr[2], Arr[4])`，`(Arr[0], Arr[4])`的按位与值不为零。 这三个边形成一个循环。
