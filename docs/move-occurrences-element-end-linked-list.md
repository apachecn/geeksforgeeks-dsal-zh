...
我们从链表的开头开始上述两个指针。 仅当`pKey`未指向键时，才移动`pKey`。 我们总是移动`pCrawl`。 因此，当`pCrawl`和`pKey`不相同时，我们必须找到一个位于`pCrawl`之前的键，因此我们交换`pCrawl`和`pKey`的数据，然后将`pKey`移动到下一个位置。 循环不变是交换数据后，从`pKey`到`pCrawl`的所有元素都是键。
