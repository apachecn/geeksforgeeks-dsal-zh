...
给定 2D 矩阵和一组单元格索引，例如`(i, j)`的数组，其中`i`是行和`j`是列。 对于每个给定的单元格索引`(i, j)`，查找除第`i`行和/或第`j`列中存在的元素之外的所有矩阵元素的和。
...
2.  计算各个行和列的总和。 （`row[]`和`col[]`）

3.  对于单元格索引`(i, j)`，所需的总和将为`sum = row[i] – col[j] + arr[i][j]`。
