...
这个想法是预先计算所有大小为 **2 <sup>j</sup>** 的子阵列的最小值，其中 j 从 0 到 **Log n** 变化。 我们进行表 lookup [i] [j]的查找，使 lookup [i] [j]包含从 i 开始且范围为 2 <sup>j</sup> 的最小值。 例如，lookup [0] [3]包含范围为[0，7]的最小值（从 0 开始且大小为 2 <sup>3</sup> ）
