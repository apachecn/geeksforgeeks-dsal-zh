...
我们可以在 `O(n^2)`时间内解决此问题。 此问题主要是[这个打印所有金额的](https://www.geeksforgeeks.org/given-n-x-n-square-matrix-find-sum-sub-squares-size-k-x-k/)问题的扩展。 这个想法是预处理给定的方阵。 在预处理步骤中，计算临时方阵 stripSum [] []中所有大小为 k x 1 的垂直条的总和。 一旦我们获得了所有垂直条带的总和，我们就可以计算该行中第一个子正方形的总和作为该行中前 k 个条带的总和，对于剩余的子正方形，我们可以通过去除`O(1)`时间来计算总和 前一个子正方形的最左边的条，并添加新正方形的最右边的条。
以下是上述想法的实现。
