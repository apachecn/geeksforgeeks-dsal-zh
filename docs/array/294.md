...
2.  如果总和为奇数，则返回`false`。
3.  使用这里讨论的基于散列的方法[在这里](https://www.geeksforgeeks.org/write-a-c-program-that-given-a-set-a-of-n-numbers-and-another-number-x-determines-whether-or-not-there-exist-two-elements-in-s-whose-sum-is-exactly-x/)作为方法 2，找到总和等于`sum / 2`的当前对。如果找到一对，请打印并返回`true`。
