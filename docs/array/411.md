...
**更好的解决方案**是[使用时间复杂度为`O(n ^ 1.58)`的分而治之找到元素](https://www.geeksforgeeks.org/divide-conquer-set-6-search-row-wise-column-wise-sorted-2d-array/)。 有关详细信息，[请参考这里](https://www.geeksforgeeks.org/divide-conquer-set-6-search-row-wise-column-wise-sorted-2d-array/)。
