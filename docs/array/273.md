...
## [推荐：请先在“ ***实践*** ”上解决它，然后再继续解决。](https://practice.geeksforgeeks.org/problems/find-triplets-with-zero-sum/1)

**方法 1**： 这是一种简单的方法，需要 O（n <sup>3</sup> ）时间才能得出结果。
...
**方法 2**： 第二种方法使用哈希处理来获得结果，并在较短的 O（n <sup>2</sup> ）上求解。
...
**方法 3**： 该方法使用排序来得出正确的结果，并以 O（n <sup>2</sup> ）的时间求解。
