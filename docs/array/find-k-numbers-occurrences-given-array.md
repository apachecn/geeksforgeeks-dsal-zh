...
*   `Hashmap`：`HashMap`是 Java 1.2 以来的 Java 集合的一部分。 它提供了 Java `Map`接口的基本实现。 它以（键，值）对存储数据。 要访问一个值，必须知道其键。 `HashMap`被称为哈希映射，因为它使用了一种称为哈希的技术。 哈希是一种将大字符串转换为代表相同字符串的小字符串的技术。 较短的值有助于索引编制和更快的搜索。 `HashSet`还内部使用`HashMap`。 它在内部使用链表来存储已在`HashSet`中详细解释的键值对以及其他文章。
...
*   **复杂度分析**：
 ...
 [**在线性时间中查找最常见的`k`个**](https://www.geeksforgeeks.org/find-k-most-frequent-in-linear-time/)
