...
给定两个数组 a []和 b []，我们需要构建一个数组 c []，使得 c []的每个元素 c [i]都包含 a []中的一个值，该值大于 b [i]并且最接近 到 b [i]。 如果 a []的元素不大于 b [i]，则 c [i]的值为-1。 所有阵列的大小相同。
...
**有效方法：**对数组 a []进行排序，并对每个 b [i]在排序后的数组 a []中应用[二元搜索](https://www.geeksforgeeks.org/binary-search/)。 对于这种方法，我们的时间复杂度将为 O（nlogn）。
