...
## [推荐：请先在“ ***<u>实践</u>*** ”上解决它，然后再继续解决。](https://practice.geeksforgeeks.org/problems/count-distinct-pairs-with-difference-k/0)

**方法 1（简单）**
一个简单的解决方案是逐一考虑所有对，并检查每对之间的差异。 以下程序实现简单的解决方案。 我们运行两个循环：外部循环选择对的第一个元素，内部循环寻找另一个元素。 如果阵列中有重复项，则此解决方案将不起作用，因为要求仅计算不重复的对。
