...
由于此方法是**脱机的**（我们的意思是[脱机](https://www.geeksforgeeks.org/median-of-stream-of-integers-running-integers/)吗？），因此我们不确定是否添加 8 将扩展该系列。 假设输入数组中有 9 个，例如`{2, 5, 3, 7, 11, 11, 8, 7, 9, …}`。 我们可以将 8 替换为 11，因为可能存在**最佳**候选（9），可以扩展新系列`{2, 3, 7, 8}`或`{2, 5, 7, 8}`。
