...
## [推荐：请先在“ ***<u>实践</u>*** ”上解决它，然后再继续解决。](https://practice.geeksforgeeks.org/problems/subarray-with-given-sum/0)

<u>**简单方法：**</u> 一个简单的解决方案是一个一个地考虑所有子数组并检查每个子数组的总和。 以下程序实现简单的解决方案。 运行两个循环：外部循环选择起点 I，内部循环尝试从 i 开始的所有子数组。
...
<u>**有效方法：**</u> 有一个想法，即数组的所有元素都是正数。 如果子数组的总和大于给定的总和，则不可能将元素添加到当前子数组中，总和为 *x* （给定总和）。 想法是对滑动窗口使用类似的方法。 从一个空的子数组开始，向该子数组添加元素，直到总和小于 *x* 为止。 如果总和大于 *x* ，则从当前子数组的开头删除元素。
...
*   [查找具有给定总和的子数组| 设置 2（处理负数）](https://www.geeksforgeeks.org/find-subarray-with-given-sum-in-array-of-integers/)
