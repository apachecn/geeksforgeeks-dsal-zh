...
> [推荐：请先在“ ***实践*** ”上解决它，然后再继续解决。](https://practice.geeksforgeeks.org/problems/largest-number-formed-from-an-array/0)
...
给定两个数字 **X** 和 **Y**，`myCompare()`应该如何确定要放哪个数字–我们比较两个数字 XY（Y 附加在 X 的末尾 ）和 YX（X 附加在 Y 的末尾）。 如果 **XY** 较大，则在输出中 X 应该位于 Y 之前，否则 Y 应该位于 Y 之前。 例如，令 X 和 Y 为 542 和 60。要比较 X 和 Y，我们比较 54260 和 60542。由于 60542 大于 54260，因此我们将 Y 放在第一位。
