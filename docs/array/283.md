...
一个简单的解决方案是线性遍历数组。 简单解决方案的时间复杂度为`O(n)`。 我们可以使用 [Binary Search](http://quiz.geeksforgeeks.org/binary-search/) 查找`O(Logn)`时间的计数。 这个想法是使用二分搜索查找 1 的最后一次出现。 一旦找到索引最后一次出现，我们将返回`index +1`作为计数。
