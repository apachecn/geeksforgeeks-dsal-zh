...
关键思想是构建一个[段树](https://www.geeksforgeeks.org/segment-tree-set-1-sum-of-given-range/)，每个节点上都有一个向量，并且该向量按排序顺序包含子范围的所有元素。 而且，如果我们观察到这种段树结构，则该结构与[合并排序算法](https://www.geeksforgeeks.org/merge-sort/)期间形成的树类似（这就是为什么将其称为合并排序树的原因）
...
因此，通过在索引上建立合并排序树，我们可以得到`L`到`R`范围内的第`K`个最小数字的查询，形式为 `O(n(logn)^2)`。
