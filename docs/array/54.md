...
## [推荐：请先在“ ***<u>实践</u>*** ”上解决它，然后再继续解决。](https://practice.geeksforgeeks.org/problems/largest-subarray-of-0s-and-1s/1)

**<u>方法 1</u> ：**蛮力。

**方法：**这些类型问题中的蛮力方法是生成所有可能的子数组。 然后，首先检查子阵列的 **0 的**和 **1 的**数目是否相等。 为了简化此过程，请按 **0 分别为-1** 和 **1 的**的子阵列的**累积总和**。 **累积总和= 0** 的点表示从开始到该点的子数组具有相等数量的 **0 的**和 **1 的**。 现在，由于这是一个有效的子阵列，请将其大小与迄今为止找到的此类子阵列的最大大小进行比较。
...
5.  如果累积总和**（总和的值）= 0** ，则表示子数组的**的数量等于 0 和 1 的**。
6.  现在，将其大小与最大子数组的大小进行比较，如果更大，则将此类子数组的第一个索引存储在变量中并更新 size 的值。
...
*   **时间复杂度：** O（n ^ 2）。
    由于所有可能的子阵列都是使用一对嵌套循环生成的。
*   **辅助空间：** O（1）。
    因为没有使用占用辅助空间的额外数据结构。

**<u>方法 2：</u>** [Hashmap](http://www.geeksforgeeks.org/java-util-hashmap-in-java/) 。

**方法：**以**累积和，以 0 为-1 的概念**将有助于我们优化该方法。 在求和时，有两种情况可以有一个子数组，其子数组的个数分别为 0 和 1。

1.  当**累积总和= 0** 时，表示从索引**（0）**到当前索引的子数组具有相等数量的 **0 和 1** 。
2.  当我们遇到之前已经遇到的累加总和值时，这意味着从**先前索引+1** 到**当前索引**的子数组具有等于 0 和 1 的数量 他们给出**的总和为 0** 。
