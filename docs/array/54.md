...
**方法**：这些类型问题中的蛮力方法是生成所有可能的子数组。 然后，首先检查子数组的 **0 的**和 **1 的**数目是否相等。 为了简化此过程，请按 **0 分别为-1** 和 **1 的**的子数组的**累积总和**。 **累积总和= 0** 的点表示从开始到该点的子数组具有相等数量的 **0 的**和 **1 的**。 现在，由于这是一个有效的子数组，请将其大小与迄今为止找到的此类子数组的最大大小进行比较。
