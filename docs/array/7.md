...
在这里，type 指定要分配的数据的类型，size 指定数组中元素的数量，Name_Array 是数组变量的名称。 并且**新的**将根据其大小为阵列分配内存。
...
多维数组包含多个行来存储值。 在 [C＃](https://www.geeksforgeeks.org/introduction-to-c-sharp/)中也称为**矩形阵列**，因为它的每行长度都是相同的。 它可以是 **2D 阵列**或 **3D 阵列**或更多。 为了存储和访问数组的值，需要嵌套循环。 多维数组的声明，初始化和访问如下：
...
元素为数组的数组称为锯齿数组，其含义是“ **数组**”。 锯齿状的阵列元件可以具有不同的尺寸和大小。 以下示例显示了如何声明，初始化和访问锯齿状数组。
...
可以混合锯齿状和多维阵列。 锯齿状数组是数组的数组，因此其元素是引用类型，并且被初始化为 null。
