...
**<u>有效方法</u>** 该过程可能始于将数组合并为两个组。 第一次合并后，我们有 k / 2 个数组。 再次合并成组的数组，现在我们有了 k / 4 个数组。 这类似于合并排序。 将 k 个数组分成两个相等的数组，直到数组中有两个数组为止。 随后以自底向上的方式合​​并数组。
