...
给定一个包含 **n** 整数和值 **d** 的数组。 给出 **m 个**查询。 每个查询具有两个值**开始**和**结束**。 对于每个查询，问题是将给定数组中从**开始**到**结束**索引的值增加给定值 **d** 。 需要线性省时的解决方案来处理此类多个查询。
...
**天真的方法：**对于每个查询，遍历**开始**到**结束**范围内的数组，并将该范围内的值增加给定值 **d** 。

**有效方法：**创建大小为 **n** 的数组 **sum []** 并将其所有索引初始化为 0。现在，对于每个**（开始， end）**索引对在 **sum []** 数组上应用给定的运算。 仅当存在索引**（end + 1）**时，操作才是 **sum [start] + = d** 和 **sum [end + 1]-= d** 。 现在，从索引 **i** = 1 到 n-1，将 **sum []** 数组中的值累加为： **sum [i] + = sum [i-1 ]** 。 最后，对于索引 **i** = 0 到 n-1，执行以下操作： **arr [i] + = sum [i]** 。
