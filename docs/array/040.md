...
上述解决方案的时间复杂度为`O（n log n）`。 这种方法的问题是我们正在使用辅助数组进行合并，但不允许使用任何数据结构来解决此问题。 我们可以在不使用任何数据结构的情况下进行就地合并。 这个想法来自这里。

令`Ln`和`Lp`分别表示左子阵列的负部分和正部分。 类似地，`Rn`和`Rp`分别表示右子阵列的负和正部分。
