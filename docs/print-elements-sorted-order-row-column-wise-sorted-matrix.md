...
提取最小值的时间复杂度为`O(n)`，称为 `O(n^2)`倍。 因此，总时间复杂度为`O(N ^ 3)`。

**更好的解决方案**是使用方法来[合并`k`个排序的数组](https://www.geeksforgeeks.org/merge-k-sorted-arrays/)。 想法是使用大小为`N`的最小堆来存储第一列的元素。 做最小提取。 在最小提取中，将最小元素替换为要从中提取元素的行的下一个元素。 该解决方案的时间复杂度为`O(N ^ 2 LogN)`。
