...
**解决方案**：因此，主要动机是找到一个元素，该元素的元素在数组的左侧小于其自身，而元素的大小大于在数组的右侧，如果有的话 这样的元素存在一个满足标准的三元组。

**方法**：这可以通过非常简单的方式解决。 要查找数组左侧的元素小于其自身的元素，请在从起始索引即（0）开始遍历数组时检查该元素是否为最小元素，并检查是否存在大于 本身在数组的右侧，从该数组的末尾遍历（`n-1`）时，检查该元素是否为最大元素。 如果该元素不是从 0 到该索引的最小元素，则它的左侧小于其自身的元素；类似地，如果该元素不是从该索引到最后一个索引的最大元素，则其上的元素更大 它的右侧。
