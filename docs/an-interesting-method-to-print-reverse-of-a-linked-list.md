...
在本文中，我们讨论了一种有趣的方法，该方法不需要递归，也不需要修改列表。 该函数也只访问链表的每个节点一次。

**技巧**：以反向顺序打印列表而没有任何递归函数或循环的想法是使用[回车符](https://en.wikipedia.org/wiki/Carriage_return)（`"r"`）。 为此，我们应该了解列表的长度。 现在，我们应该打印`n-1`个空白字符，然后打印第一个元素，然后打印`"r"`，再打印`n-2`个空白字符，再打印第二个节点，然后打印`"r"`，依此类推。
