...
**方法**：每个元素和`x`的二进制表示，我们可以观察到每个位都是独立的，并且可以通过迭代每个位来解决问题。 现在基本上，对于每个位，我们都需要计算给定范围内的 1 和 0 的数量，如果 1 的数量更多，则必须将`x`的该位设置为 0，以便在`x`或`x`之后用`x`取最大，否则 0 的数量更多，那么您必须将`x`的该位设置为 1。如果 1 和 0 的数量相等，那么我们可以将`x`的该位设置为 1 或 0 中的任何一个，因为这不会影响总和，但是我们必须最小化`x`的值，因此我们将使用该位 0。
