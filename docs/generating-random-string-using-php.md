...
要了解有关这些函数的更多信息，[请单击此处](https://www.geeksforgeeks.org/php-md5-sha1-hash-functions/)。

一旦我们了解了如何利用这些函数，我们的任务就会变得非常简单。
...
**注意**：所有上述方法都是基于`rand()`和`uniqid()`函数构建的。 这些函数不是加密安全的随机生成器。 因此建议，如果随机性程度影响应用的安全性，则应避免使用这些方法。
