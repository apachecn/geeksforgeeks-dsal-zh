...
给定整数`K`，使得存在一组所有可能的[完全平方](https://www.geeksforgeeks.org/check-if-given-number-is-perfect-square-in-cpp/)，每个长度为`K`。 从这组完美的正方形中，生成一组具有尽可能多的长度的集合，其长度彼此为异序词。 任务是打印在生成的异序词集中存在的最大元素。
...
**朴素的方法**：最简单的方法是存储所有可能的`K`个长度的完全平方，并使用[递归](http://www.geeksforgeeks.org/recursion/)生成有效的七字组。 然后，找到最长长度集中存在的最大元素。
