# 给定一个已排序的数组和一个数字`x`，在数组中找到总和最接近`x`的偶对
...
一个有效的解决方案可以在`O(n)`的时间内找到该对。 这个想法类似于此帖子的[方法 2](https://www.geeksforgeeks.org/write-a-c-program-that-given-a-set-a-of-n-numbers-and-another-number-x-determines-whether-or-not-there-exist-two-elements-in-s-whose-sum-is-exactly-x/)。 以下是详细的算法。
