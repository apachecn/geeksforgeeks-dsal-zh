...
在上一篇文章[跳表 | 系列 2（插入）](https://www.geeksforgeeks.org/skip-list-set-2-insertion/)中，我们讨论了跳过节点的结构以及如何在跳表中插入元素。 在本文中，我们将讨论如何在跳表中搜索和删除元素。
...
在删除元素`k`之前，使用上述搜索算法在跳表中定位元素。 定位元素后，就像我们在单链表中一样，完成了指针的重新排列以删除元素表单列表。 我们从最低级别开始进行重新排列，直到`update[i]`旁边的元素不是`k`。

删除元素后，可能会存在没有元素的级别，因此我们也将通过减少跳表的级别来删除这些级别。 以下是删除的伪代码：
