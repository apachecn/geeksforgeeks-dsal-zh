...
3.  查询 3 是类型 2：如此类查询的定义中所述，我们将执行范围内所述的查询，即，我们将操作查询而不是数组。 给定的范围是 1 和 2，所以我们将再次执行查询 1 和 2，即，我们将对类型 2 查询使用重复方法，因此我们将再次执行查询 1，数组将是`2 2 0 11`。现在，我们将执行查询 2，结果数组将是`2 2 0 2 2`。
...
在此方法中，我们使用一个额外的数组来创建记录数组，以查找执行特定查询的时间，并且在创建记录数组之后，我们仅执行类型1的查询，并将记录数组的包含内容简单地添加到主数组，这将给我们结果数组。
