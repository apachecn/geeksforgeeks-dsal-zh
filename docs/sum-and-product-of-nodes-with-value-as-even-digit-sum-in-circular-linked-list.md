...
给定一个包含`N`个节点的循环单链表，任务是从列表中找到其数据值具有偶数和的所有节点的和与乘积。

**示例**：

> **输入**：`list = 15 -> 16 -> 8 -> 6 -> 13`
>
> **输出**：`sum = 42, product = 9360`
>
> **解释**：
>
> 循环链表包含：
...
1.  用循环链表的开头初始化指针`curr`，并将和变量`sum`设为 0，并将乘积变量`product`设为 1。

2.  开始使用`do-while`循环遍历链表，直到遍历所有节点。
