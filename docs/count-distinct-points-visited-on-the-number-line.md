...
*   遍历字符串`path`并执行以下操作：
    ...
    *   如果当前字符为`'0'`，则此人向左走。 因此，将当前位置减小`1`并将其存储在`points[]`中。

    *   如果当前字符为`'1'`，则此人向右行驶。 因此，将当前位置增加`1`并将其存储在`points[]`中。

*   计算`points[]`中不同元素的总数。 有关对数组中的不同元素进行计数的不同方法，请参考[对数组中的不同元素进行计数](https://www.geeksforgeeks.org/count-distinct-elements-in-an-array/)。
