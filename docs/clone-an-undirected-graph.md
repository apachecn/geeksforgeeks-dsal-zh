...
在访问节点`u`的相邻顶点时获得相应克隆节点，我们称`cloneNodeU`，现在访问`u`的所有相邻节点，为每个邻居找到对应的克隆节点（如果找不到，请创建一个），然后将其推入`cloneNodeU`节点的相邻向量中。
...
[克隆具有多个连通组件的无向图](https://www.geeksforgeeks.org/clone-an-undirected-graph-with-multiple-connected-components/)
