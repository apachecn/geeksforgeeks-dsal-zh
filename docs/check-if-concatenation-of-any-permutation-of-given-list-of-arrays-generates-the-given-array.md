...
给定`N`个不同整数的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)`arr[]`以及一个不同整数的数组`pieces[]`的列表， 检查给定的数组列表是否可以以任何顺序连接以获得给定的数组。 如果可能，则打印`Yes`。 否则，打印`No`。
