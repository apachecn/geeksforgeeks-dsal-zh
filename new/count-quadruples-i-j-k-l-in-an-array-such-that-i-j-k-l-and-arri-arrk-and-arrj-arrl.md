# 计数数组中的四元组`(i, j, k, l)`数量, 其中`i < j < k < l`和`arr[i] = arr[k]`和`arr[j] = arr[l]`
...
给定由`N`个整数组成的[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)`arr[]`, 任务是计算给定数组中的元组*`(i, j, k, l)`的数量, 使得`i < j < k < l`和`arr[i] = arr[k]`和`arr[j] = arr[l]`。
...
> 4.  `(1, 3, 4, 5)`，因为`arr[1] = arr[4] = 2`且`arr[3] = arr[5] = 2`
> 
> **输入**：`arr[] = {2, 5, 2, 2, 5, 4}`
>
> **输出**：2

**朴素的方法**：最简单的方法是生成所有可能的四倍体, 并检查给定条件是否成立。 如果发现是真的, 则增加最终计数。 打印获得的最终计数。
