# 从列表中删除可被给定数字K整除的所有节点

给定一个链表和一个密钥K。任务是编写一个程序，删除列表中所有可被K整除的节点。
**示例：**
...
1.  头节点的值可被K整除：首先检查头节点上所有可被K整除的事件，删除它们并适当地更改头节点。
2.  一些中间节点持有可被k整除的值：从头开始遍历并检查当前节点的值是否可被K整除。如果是，则删除该节点并在列表中向前移动。
