...
给定`N`大小的数组`arr[]`，然后是以下两种类型的数组的`Q`查询：

*   查询类型 1：给定两个整数`L`和`R`，找到从索引`L`到`R`的质数元素的总和，其中`0 <= L <= R <= N-1`。

*   查询类型 2：给定两个整数`i`和`X`，更改`arr[i] = X`，其中`0 <= i <= n-1`。

**注意**：每个子查询的第一个索引确定要回答的查询的类型。
...
>
> 第三个查询的类型为 1，其中`arr[1] = 10`，它不是质数，因此答案为`5 + 7= 12`
> 
...
>
> 第二个查询的类型为 1，因为`arr[4] = 3`，这是质数。 所以答案是`3 + 11= 14`

**朴素的方法**：的想法是针对`L`到`R`之间的每个查询进行迭代，并在给定的数组上执行所需的操作。
