...
对给定的生物张力双链表排序。 双调双向链表是一个双向链表，它首先增加然后减小。 严格增加或严格减少的列表也是生物群落双链表。

**示例**：

![](img/381780281bb7d4df31529363d184a31d.png)

**方法**：在列表中找到小于前一个节点的第一个节点。 使其为`curr`。 如果不存在这样的节点，则列表已经排序。 其他将列表分为两个列表，`first`从**头**节点开始直到`curr`的先前节点，`second`从`curr`节点开始直到列表的末尾。 颠倒`second`双链表。 请参阅此帖子。 现在合并`first`和`second`排序的双链表。 请参阅此帖子的[合并程序](https://www.geeksforgeeks.org/merge-sort-for-doubly-linked-list/)。 最终的合并列表是所需的排序的双链表。
