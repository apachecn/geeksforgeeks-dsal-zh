...
> **输入**：`N = 2, M = 1, row[] = {-1, 1}, segment[][4] = {0, 1, 2, 1}`
>
> **输出**：1
>
> **说明**：
...
> 
> **输入**：`N = 4, M = 8, row[] = {-5, -3, 2, 3}, segment[][4] = {{-2, 5, 5, -6 }, {-5, -2, -3, -5}, {-2、3, -6、1}, {-1, -3、4、2}, {2、5、2、1},  {4, 5, 4, -5}, {-2, -4, 5, 3}, {1, 2, -2, 1}}`
>
> **输出**：8
>
> **说明**：
>
> 共有 8 个交点。虚线是垂直线。绿色圆圈表示单个相交点，绿色三角形表示两条线段，在该点处与同一垂直线相交。
> 
> ![Example 2 Image](img/3424ad00aa9457cb45104c7413464d60.png)

**朴素的方法**：

最简单的方法是，对于每个查询，检查垂直线是否落在两点的`x`坐标之间。 因此，每个段将具有`O(n)`个计算复杂度。
...
*   首先，计算到现在为止垂直线出现次数的前缀数组，然后仅减去出现次数直到`x2-1`（我们不考虑`x2`，因为它仅表示为触碰而不是相交，从出现次数到`x1`为止。 因此，对于每个段，计算复杂度降低为`O(1)`。
...
**方法 3**：为了优化上述方法，我们可以使用[映射数据结构](https://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)来节省空间。 请按照以下步骤解决问题：
