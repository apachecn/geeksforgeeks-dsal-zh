...
方法 1 构造了从根到叶的树。 在这种方法中，我们从叶到根进行构建。 想法是按照与出现在“双向链接列表”中的顺序相同的顺序在 BST 中插入节点，以便可以以 O（n）时间复杂度构造树。 我们首先计算给定链接列表中的节点数。 让计数为 n。 在计数节点之后，我们取左 n / 2 个节点并递归构造左子树。 构造左子树后，我们将中间节点分配给根，并将左子树与根链接。 最后，我们递归构造正确的子树并将其与根链接。
