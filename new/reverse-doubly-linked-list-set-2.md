...
**方法**：在[上一篇文章](https://www.geeksforgeeks.org/reverse-a-doubly-linked-list/)中，我们反转双链表，通过交换所有节点的上一个指针和下一个指针，更改头的`prev`（或开始），最后更改头的指针。 在本文中，我们创建一个`push`函数，将给定节点添加到给定列表的开头。 我们遍历原始列表，然后将当前节点指针一一传递给`push`函数。 此过程将反转列表。 最后返回此反向列表的新头部。
