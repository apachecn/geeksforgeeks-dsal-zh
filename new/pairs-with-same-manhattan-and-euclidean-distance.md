...
在给定的笛卡尔平面中，有`N`个点。 任务是找到点对的数量`(A, B)`使得：
...
> 曼哈顿距离为`|x2 - x1| + |y2 - y1|`
> 
> 欧氏距离为`((x2 - x1) ^ 2 + (y2 - y1) ^ 2) ^ 0.5`其中点是`(x1, y1)`和`(x2, y2)`。

**示例**：

> **输入**：`N = 3, points = {{1, 2}, {2, 3}, {1, 3}}`
>
> **输出**：2
>
> 偶对是：
...
1.  映射`X`，其中`X[x[i]]`存储其`x`坐标等于`x[i]`的点的数量。

2.  映射`Y`，其中`Y[y[i]]`存储其`y`坐标等于`y[i]`的点的数量。

3.  映射`XY`，其中`XY[(x[i], y[i])]`存储与点`(x[i], y[i])`重合的点数。

现在，对于所有不同的`x[i]`，

令`Xans`为具有相同`X`坐标的对数`X[x[i]] ^ 2`

设`Yans`为具有相同`Y`坐标的对数`Y[y[i]] ^ 2`对于所有不同的`y[i]`

令`XYans`为重合点数`XY[(x[i], y[i])] ^ 2`，对于所有不同点`(x[i], y[i])`
