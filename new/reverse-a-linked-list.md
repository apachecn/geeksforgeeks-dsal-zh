...
1.  初始化三个指针，前一个为`NULL`，`curr`为`head`，`next`为`NULL`。
2.  迭代链表。 循环执行以下操作。
    
    在更改`curr->next`之前，存储`next`节点。
    
    `next = curr->next`
    
    现在更改`curr-next`，这是实际发生反转的地方 。
    
    `curr->next = prev`
    
    将`prev`和`curr`向前移动一个步骤。
...
*   将节点（值和地址）存储在栈中，直到输入所有值。

*   完成所有输入后，将`Head`指针更新到最后一个位置（即最后一个值）。

*   开始弹出节点（值和地址）并以相同顺序存储它们，直到栈为空。

*   用`NULL`更新栈中最后一个节点的下一个指针。
