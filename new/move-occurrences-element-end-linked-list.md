...
一个**简单解决方案**是一个一个地找到链表中所有给定键的出现。 对于每个发现的事件，将其插入末尾。 直到所有出现的给定键都移到结尾为止。
...
`pKey`：如果找到了键，则指向发生键的指针。 其他与`pCrawl`相同。

我们从链表的开头开始上述两个指针。 仅当`pKey`未指向键时，才移动`pKey`。 我们总是移动`pCrawl`。 因此，当`pCrawl`和`pKey`不相同时，我们必须找到一个位于`pCrawl`之前的键，因此我们交换`pCrawl`的数据 和`pKey`，然后将`pKey`移动到下一个位置。 循环不变是交换数据后，从`pKey`到`pCrawl`的所有元素都是键。
...
2.  现在，检查键和节点->数据是否相等，将节点移至倒数第二个，否则向前移
...
**高效解决方案 3**：将维护单独的键列表。 我们将此键列表初始化为空。 我们遍历给定的列表。 对于找到的每个键，我们将其从原始列表中删除，然后插入单独的键列表中。 最后，我们在剩余给定列表的末尾链接关键字列表。 该解决方案的时间复杂度也是`O(n)`，并且它只需要遍历列表。
