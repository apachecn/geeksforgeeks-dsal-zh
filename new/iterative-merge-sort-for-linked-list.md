...
合并排序通常是对链表进行排序的首选。 这里在中讨论[。 但是，上面讨论的方法使用栈来存储递归调用。 如果要排序的链表太大，可能会占用大量内存。 因此，本文讨论了一种用于合并排序的纯迭代方法，无需递归调用。](https://www.geeksforgeeks.org/merge-sort-for-linked-list/)
