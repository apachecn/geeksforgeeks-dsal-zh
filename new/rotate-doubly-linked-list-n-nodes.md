...
要旋转双链表，我们需要将第`N`个节点的下一个更改为`NULL`，将最后一个节点的下一个更改为上一个头节点，将头节点的上一个更改为最后一个节点，最后将头更改为第`N + 1`个节点，然后将新的头节点的上一个设置为`NULL`（双链表中的头节点的上一个为`NULL`）

因此，我们需要掌握三个节点：第`N`个节点，第`N + 1`个节点和最后一个节点。 从头开始遍历列表，并在第`N`个节点处停止。 存储指向第`N`个节点的指针。 我们可以使用`NthNode->next`获得第`N + 1`个节点。 继续遍历直到结束，并同时存储最后一个节点的指针。 最后，使用`PrintList`功能更改上述指针，并更改`Last Print Rotated List`的指针。
