...
**方法**：的想法是将水平距离相同的节点放在一起。 我们执行[级别顺序遍历](https://www.geeksforgeeks.org/level-order-tree-traversal/)，以便先访问水平节点上的最高节点，然后再访问其下相同水平距离的任何其他节点，然后继续求和它们的值并将结果存储在变量 **sum 中** 。 散列用于检查是否看到给定水平距离的节点。
