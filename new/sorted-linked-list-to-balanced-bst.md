...
方法 1 从根到叶构造树。 在这种方法中，我们从叶到根进行构建。 想法是按照与在“链表”中出现的顺序相同的顺序在 BST 中插入节点，以便可以以`O(n)`时间复杂度构造树。 我们首先计算给定链表中的节点数。 让计数为 n。 在计数节点之后，我们取左 n / 2 个节点并递归构造左子树。 构造左子树后，我们为 root 分配内存，并将左子树与 root 链接。 最后，我们递归构造正确的子树并将其与根链接。
