...
`set(key, value)` – 如果键不存在，则设置或插入值。 当缓存达到其容量时，它应在插入新项目之前使最近最少使用的项目无效。
...
（a）对于`get(key)`：我们可以简单地遍历数组，并将每个节点的键与给定键进行比较，然后返回值 存储在该键的节点中。 如果找不到任何这样的节点，则简单地返回 -1。

时间复杂度：`O(n)`

（b）对于`set(key, value)`：如果数组已满，则必须从数组中删除一个节点。 为了找到 LRU 节点，我们将遍历数组并找到 timeStamp 值最小的节点。 我们将简单地在 LRU 节点的位置插入新节点（具有新的键和值）。
