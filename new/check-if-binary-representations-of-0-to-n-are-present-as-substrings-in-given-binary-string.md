# 检查在给定的二进制字符串中，子字符串是否存在 0 到`N`的二进制表示形式
...
给出二进制字符串`str`和整数`N`，任务是检查字符串的子字符串是否包含小于或等于给定整数`N`的所有非负整数的二进制表示形式 。
...
> **说明**：
>
> 由于子串`"0"`，`"1"`，`"10"`和`"11"`可以由给定的字符串生成。 因此，所有 0 到 3 的二进制表示形式都以给定二进制字符串中的子字符串形式出现。
> 
> **输入**：`str = "0110", N = 4`
>
> **输出**：`False`
...
*   初始化`map[]`以标记字符串，并使用位集变量`ans`将数字从十进制转换为二进制。

*   再将一个变量计数设为零。

*   使用变量`i`从`N`到`1`运行循环，并检查对应的数字是否在映射中标记。

*   如果`map`中没有标记`i`，则使用位集变量`ans`将当前数字转换为二进制。
...
*   循环运行，除非未标记`i`并且二进制数变为零：
    ...
    *   右移转换后的数字。 这样做是因为，如果将任何字符串`x`转换为二进制（例如`111001`）并且该子字符串已在映射中标记，则`11100`将已经是**自动标记**。

        这是基于以下事实：如果存在`i`，则`i >> 1`也存在。

*   最后检查计数是否是`N + 1`，然后打印`True`，否则打印`False`。
