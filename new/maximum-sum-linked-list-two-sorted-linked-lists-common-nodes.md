...
给定两个排序的链表，构造一个链表，该链表包含从头到尾的最大求和路径。 结果列表可能包含两个输入列表中的节点。 构造结果列表时，我们可能仅在交点处切换到另一个输入列表（这意味着列表中具有相同值的两个节点）。 允许您使用O（1）多余的空间。
...
3.此后，直到两个列表的当前指针都不为NULL，我们需要根据较大的和调整下一个上一个指针。
