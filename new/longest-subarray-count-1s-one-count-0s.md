...
2.  初始化`sum = 0`和`maxLen = 0`。

3.  创建具有`(sum, i)`元组的哈希表。

4.  对于`i = 0`到`n - 1`，执行以下步骤：

    1.  如果`arr[i]`为 0，则将 -1 累加至`sum`，否则将 1 则累加至`sum`。

    2.  如果`sum == 1`，则更新`maxLen = i + 1`。

    3.  否则检查哈希表中是否存在`sum`。 如果不存在，则将其作为`(sum, i)`对添加到哈希表中。

    4.  检查哈希表中是否存在`sum - 1`。 如果存在，则从哈希表中获取`sum - 1`的索引作为`index`。 现在检查`maxLen`是否小于`i - index`，然后更新`maxLen = i - index`。
