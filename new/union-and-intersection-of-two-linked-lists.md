...
遍历列表 2。 如果结果中已经存在 list2 元素，则不要将其插入到 result 中，否则插入。
...
*   **时间复杂度：** O（m * n）。
    ...
    **对于联合：**对于列表 2 中的每个元素，我们检查该元素是否已存在于使用列表 1 生成的结果列表中。
...
*   **时间复杂度：** O（m + n）。
    ...
    **对于交集：**首先遍历 list-1，将其元素存储在 Hash-map 中，然后对于 list-2 中的每个元素，检查其是否已存在于地图中。 这需要 O（1）时间。
