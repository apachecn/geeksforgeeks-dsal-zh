...
**第二次迭代**：我们继续寻找包含最小距离的非盲顶点，即顶点`1`，然后将其邻居的权重减小为 **1 + 4 = 5** 并像原始 Dijkstra 的算法一样更新其父级，并将其 **pathlength []** 设置为`2`，因为它与源顶点相距两个边。
最后，我们使顶点`1`变黑。
...
**第四次迭代**：在此迭代中，该算法的作用与原始 Dijkstra 的算法不同。 我们寻求包含最小距离为`4`的非黑色顶点。 由于从源顶点到顶点`3`的距离是 **12** （0- > 1- > 2- > 3）和顶点`4`加上边**（4，3）**是 **12** ，这意味着我们刚刚找到了从源顶点到顶点`3`的新路径 一样的重量 然后，我们检查新路径（在边上）是否比现有路径短，并选择边数最少的路径。
最后，我们使顶点`4`变黑。
