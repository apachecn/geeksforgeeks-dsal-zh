...
给定一棵树，该树由从 **[1，N]** 编号的 **N 个**节点组成，并被着色为**黑色（由 1 表示）或**绿色（由**表示） 0）**，任务是计算长度为`K`[a <sub>1</sub> ， <sub>2</sub> ，.. a <sub>K 的序列数</sub> ，使得连续节点之间的路径最短，并且覆盖的边至少由一个黑色边组成。 由于答案可能很大，因此将其打印为 **10 <sup>9</sup> +7** 的模。
...
的想法是计算长度`K`的序列数，以便不覆盖任何黑色边。 令计数为 **temp** 。 然后**（N <sup>K</sup> ）–温度**是必需的答案。 **temp** 可以通过去除黑边，然后计算所得图形的不同成分的大小来轻松计算。
