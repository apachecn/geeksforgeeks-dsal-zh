...
给定一个包含小写英文字母的字符串`S`和一个整数`K`，任务是查找包含超过`K`个以上有效字符的字符串的任何索引。 如果找到，则打印`Yes`。 否则，打印`No`。
...
*   这个想法是将出现在字符串中的每个字符的最后出现都存储在[映射](http://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)中。

*   遍历字符串并继续将活动字母存储在[集合](https://www.geeksforgeeks.org/set-in-java/)中。

*   如果**集合**的大小超过`K`，则打印`Yes`。

*   否则，检查当前索引是否是当前字符的最后一次出现。 如果是这样，请从**集合**中删除字符。

*   最后，如果没有找到超过`K`个活动字符的索引，则打印`No`。
