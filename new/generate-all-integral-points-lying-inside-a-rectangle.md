...
给定长度为`L`且宽度为`W`的矩形，其顶点之一在原点`(0, 0)`，任务是生成位于`L * W`尺寸的矩形`pf`内的所有积分坐标`(X, Y)`。
...
**方法**：可通过使用`rand()`函数，为`X`坐标生成从`0`到`L`范围内的所有整数，为`Y`坐标生成从`0`到`W`范围内的所有整数来解决该问题。 请按照以下步骤解决问题：

1.  创建[偶对的集合](https://www.geeksforgeeks.org/sets-of-pairs-in-c/)，以存储矩形内的所有坐标`(X, Y)`。

2.  使用等式`rand() % L`生成`0`至`L`之间的所有整数，使用`rand() % W`生成`0`至`W`之间的所有整数。
