...
> **输出**：1
> **说明**：
> 因为 6 和 2 可以 与总和 8（= 2 <sup>3</sup> ）形成一对。 因此，仅 11 个必须删除，因为它不构成 2 的幂的和。
> **输入**：arr [] = [1，1，1，1023]，
> **输出**：0
> **说明**：
> 给定的数组元素可以分为以下两对：
> （1、1），和 2（= 2 <sup>] 1</sup> ）
> （1，1023）和 1024（= 2 <sup>10</sup> ）
> 因此，不需要删除任何元素。
...
*   对于每个数组元素 a [i]，迭代所有可能的总和 p = {2 <sup>0</sup> ，2 <sup>1</sup> ，....，2 <sup>30</sup> }并检查是否 **p – a [i]数组中是否存在**。
...
*   需要满足以下两个条件之一：
    ...
    3.  如果以上两个条件都不满足，则由 a [i]和组成的对不可能为对。

*   如果上述两个条件对于电流 a [i]的任何 p 不满足，则增加**计数**，因为 a [i]无法与任何其他数组元素形成 2 的幂的和。
