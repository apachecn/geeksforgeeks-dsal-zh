...
> {偶数，偶数}
> 
> 请注意，任何相邻元素的{Odd，Odd}排列将给出奇数乘积。 因此，这种布置是不可能的。

以上安排仅在以下情况下可行

> **number_of_odd_elements 数组中的< = number_of_even_elements + 1** 。

请按照以下步骤解决问题。

1.  取两个向量**偶数**和**奇数**，分别存储数组的偶数和奇数元素。

2.  如果奇数矢量的**大小大于偶数矢量的大小+ 1** （如上所述），则不可能求解。 因此，打印 **-1** 。

3.  否则，首先打印**奇数**向量中的一个元素，然后打印**甚至**向量中的一个元素，直到两个向量都为空。
...
**时间复杂度**：`O(n)`，其中 N 为元质数。 遍历给定数组并形成奇数&偶数向量需要`O(n)`时间，而打印排列需要`O(n)`。
