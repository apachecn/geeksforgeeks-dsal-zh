...
> **说明：** 365 + 248 = 613
> **输入：**
> List1：7- > 5- > 9- > 4- > 6 //代表数字64957
> List2：8- > 4 //代表数字48
> **输出：**
> 结果列表：5- > 0- > 0- > 5- > 6 //代表数字65005
> **说明：** 64957 + 48 = 65005

**方法**：遍历两个列表和两个列表的一个接一个拾取节点并添加值。 如果总和大于10，则使进位为1并减少总和。 如果一个列表的元素多于另一个列表，则将该列表的剩余值视为0。

**步骤为：**

1.  从头到尾遍历两个链接列表
2.  将两个数字分别从各自的链接列表中添加。
3.  如果列表之一已到达末尾，则将0用作其数字。
4.  继续进行直到两个列表都结束。
5.  如果两个数字的总和大于9，则将进位设置为1，将当前数字设置为 *sum％10*
...
相关文章：[添加两个由链表表示的数字| 设置2](https://www.geeksforgeeks.org/sum-of-two-linked-lists/)
