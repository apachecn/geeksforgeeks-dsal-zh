...
给定一个有向加权图，该图具有`N`个顶点和`M`个边以及一个边`(U, V)`。 任务是查找是否存在从`U`到`V`的替代路径，替代路径中边的单个权重小于直接路径的权重。 如果存在，则打印`Yes`，否则打印`No`。
...
> 
> **输入**：`N = 7, M = 10, U = 3, V = 6`。 对于给定的边`{3, 6}`，权重为 16。没有替代路径可以从 3 到达 6。因此答案是`No`。
> 
> **输入**：`N = 7, M = 10, U = 1, V = 6`
>
> **输出**：`Yes`
...
> 对于给定的边`{1, 6}`，权重为 5。
>
> 从 1 到 6 的替代路径为`{1, 5, 6}`，各个权重为`{12, 2}`大于 5。因此无法考虑此路径。
>
> 从 1 到 6 的替代路径为`{1, 2, 4, 5, 6}`，且各个权重`{5, 1, 1, 2}`不小于 5。因此，该路径无法考虑。
>
> 从 1 到 6 的替代路径为`{1, 4, 5, 6}`，各个权重`{3, 1, 2}`小于 5。因此可以考虑该路径。
>
> 因此，答案是肯定的。
