...
*   **时间复杂度**：链表的合并种类花费`O(n log n)`时间。 在归并排序树中，高度为`log n`。 对每个级别进行排序将花费`O(n)`时间。 因此，时间复杂度为`O(n log n)`。

*   **辅助空间**：`O(n log n)`，在归并排序树中，高度为`log n`。 存储每个级别将占用`O(n)`空间。 因此，空间复杂度为`O(n log n)`。
