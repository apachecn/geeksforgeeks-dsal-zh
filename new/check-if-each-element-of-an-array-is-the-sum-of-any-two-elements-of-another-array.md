...
给定两个[数组](https://www.geeksforgeeks.org/introduction-to-arrays/)`A[]`和`B[]`，它们由`N`个整数组成，任务是检查数组`B[]`可以通过添加数组`A[]`的任意两个元素来形成。 如果可能，则打印`Yes`。 否则，打印`No`。
...
*   将`B[]`的每个元素存储在[集合](https://www.geeksforgeeks.org/set-in-cpp-stl/)中。

*   对于数组`A[]`的每对索引`(i, j)`，检查是否存在`A[i] + A[j]`分组。 如果发现是真的，则从**集合**中删除`A[i] + A[j]`。

*   如果集合为空，则打印`Yes`。 否则，打印`No`。
