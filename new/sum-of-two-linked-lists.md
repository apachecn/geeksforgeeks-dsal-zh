...
…。 **a）**计算两个链表的大小差异。 假设差异为*与*，

…。 **b）**将 *diff* 节点移到较大的链表中。 现在，使用步骤 2 计算较小列表和较大列表的右子列表（大小相同）的总和。 另外，存储此和的进位。
...
由于我们需要从两个链表中的最后一个开始添加数字。 因此，这里我们将使用堆栈数据结构来实现这一点。

*   我们首先将从给定的两个链表中获得两个堆栈。
