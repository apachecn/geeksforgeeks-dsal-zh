...
以下是步骤。
**1）**计算给定两个链接列表的大小。
**2）**如果大小相同，则使用递归计算总和。 将所有节点保持在递归调用堆栈中，直到最右边的节点，计算最右边的节点的总和，然后再进位到左边。
**3）**如果尺寸不相同，请执行以下步骤：
…。 **a）**计算两个链接列表的大小差异。 假设差异为*与*，
…。 **b）**将 *diff* 节点移到较大的链接列表中。 现在，使用步骤 2 计算较小列表和较大列表的右子列表（大小相同）的总和。 另外，存储此和的进位。
…。 **c）**用较大列表的剩余左子列表计算进位总和（在上一步中计算）。 此和的节点将添加到上一步获得的和列表的开头。
...
由于我们需要从两个链接列表中的最后一个开始添加数字。 因此，这里我们将使用堆栈数据结构来实现这一点。

*   我们首先将从给定的两个链接列表中获得两个堆栈。
