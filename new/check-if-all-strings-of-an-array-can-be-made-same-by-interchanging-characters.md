...
给定大小为`N`的[数组](https://www.geeksforgeeks.org/array-data-structure/)`arr[]`，该数组由相等长度的字符串组成，任务是检查是否可以通过将一个字符串的任何字符与同一字符串或另一字符串的任何字符交换，使数组的所有字符串相等。
...
**方法**：该问题可以通过计算给定数组的每个字符的[频率](https://www.geeksforgeeks.org/python-frequency-of-each-character-in-string/)并检查是否可以被`N`整除来解决。 请按照以下步骤解决问题：

1.  初始化数组`hash[256] = {0}`以存储字符的频率。

2.  遍历`hash[]`数组，并检查所有字符的频率是否可被`N`整除。

3.  如果所有字符的频率都可以被`N`整除，则打印`Yes`。
