...
*   链接所有节点的**下**指针。 对于给定的单元格，如果在其下方不存在任何单元格，则将其向下指针指向该行的最顶部单元格以保持循环顺序。

*   链接所有节点的**上**指针。 对于给定的单元格，如果在其上方不存在任何单元格，则将其向上指针指向该行的最底部单元格以保持循环顺序。
...
*   链接所有节点的**后**指针。 对于给定的单元格，如果后面没有单元格，则将其向后指针指向该行的最前面的单元格以保持循环顺序。
...
**时间复杂度**：`O(X * Y * Z)`，其中`X`，`Y`，`Z`是 3D 矩阵的大小。

**辅助空间**：`O(X * Y * Z)`，其中`X`，`Y`，`Z`是 3D 矩阵的大小。
