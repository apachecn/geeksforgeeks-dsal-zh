...
系统中有两个单链表。 由于编程错误，链接列表之一的末端节点链接到第二个列表，从而形成了一个倒Y形列表。 编写一个程序，使两个链表合并。
...
**先决条件：** [编写函数以获取两个链接列表的交点](https://www.geeksforgeeks.org/write-a-function-to-get-the-intersection-point-of-two-linked-lists/)

**方法：**将两个指针指向两个链接列表的头部。 如果其中一个提前到达末尾，则通过将其移至另一列表的开头来使用它。 一旦它们都经过重新分配，它们将与碰撞点等距。
