...
在上图中，顶点表示章节，边缘 e（u，v）表示从'u'到达'v'所要读取的页面数。 添加了接收器节点以表示书的结尾。

首先，计算一天要读取的平均页面数（此处为 31/3，大约为 10）。 新的边缘权重 e’（u，v）将是平均差| avg – e（u，v）|。 针对上述问题的修改图形为
...
这个想法是从第 1 章开始，做一个 DFS 来找到边缘数为“ k”的汇点。 继续将访问的顶点存储在一个名为“ path []”的数组中。 如果到达目标顶点，并且路径总和小于最佳路径，则更新最佳分配 optimistic_path []。 请注意，该图是 DAG，因此在 DFS 期间无需照顾周期。
