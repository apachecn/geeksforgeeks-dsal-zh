...
在这种方法中，首先，我们将对给定的二叉树进行有序遍历，并将此遍历存储在向量中，并将其与树一起传递给函数。 现在，从向量的元素生成一个循环的双链表。

要从向量生成一个循环的双链表，请将向量的第一个元素作为链表的头部，并创建一个当前指针，该指针现在指向头部。 现在，从第二个元素开始遍历数组，然后执行以下操作。
...
对所有元素执行此操作，并在完成遍历后，使`current`的右指针（`current`现在指向向量的最后一个元素）指向列表的开头，并使头部的左指针指向当前指针。 最后，返回头部。
