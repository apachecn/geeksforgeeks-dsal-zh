...
想法是将给定的蛇形和梯形板视为有向图，其顶点数等于板上的单元数。 问题减少到找到图中的最短路径。 如果下一个 6 个顶点没有蛇形或梯形，则图形的每个顶点都具有到下一个 6 个顶点的边缘。 如果接下来的六个顶点中的任何一个具有蛇或梯子，则当前顶点的边将到达梯子或蛇尾的顶部。 由于所有边缘的权重均等，因此我们可以使用图形的[广度优先搜索](https://www.geeksforgeeks.org/breadth-first-traversal-for-a-graph/)有效地找到最短路径。
