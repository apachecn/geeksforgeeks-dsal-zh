...
**高效方法**：可以通过保持上述天真的方法来优化 遍历元素的单调递减[栈](http://www.geeksforgeeks.org/stack-data-structure/)。 如果找到更大的元素，则将其附加到结果链表 **L'**上，否则将`0`附加。 步骤如下：
...
2.  一个接一个地选取其余节点，然后在循环中执行以下步骤：
    ...
    *   如果下一个节点的值大于顶部节点的值，则从栈中弹出顶部节点，而 next 是弹出节点的下一个更大的元素。
