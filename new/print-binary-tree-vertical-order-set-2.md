...
我们已经在[之前的文章](https://www.geeksforgeeks.org/print-binary-tree-vertical-order/)中讨论了`O(N ^ 2)`解决方案。 在这篇文章中，讨论了一种基于哈希图的有效解决方案。 我们需要检查所有节点到根的水平距离。 如果两个节点的水平距离（HD）相同，则它们在同一垂直线上。 HD 的想法很简单。 根的 HD 为 0，将右边缘（连接到右子树的边缘）视为+1 水平距离，而左边缘则视为-1 水平距离。 例如，在上面的树中，节点 4 的 HD 为-2，节点 2 的 HD 为-1，节点 5 和 6 的 HD 为 0，节点 7 的 HD 为+2。
