...
如果要插入新的ID 1005，则要维持排序顺序，我们必须将所有元素都移到1000（不包括1000）之后。
除非使用某些特殊技术，否则数组删除也很昂贵。 例如，要删除id []中的1010，必须移动1010之后的所有内容。
...
**链表遍历**
在上一个程序中，我们创建了一个包含三个节点的简单链表。 让我们遍历创建的列表并打印每个节点的数据。 为了遍历，让我们编写一个通用函数printList（），该函数打印任何给定的列表。
...
*   [在链表](http://quiz.geeksforgeeks.org/data-structure/linked-list/)上练习MCQ问题
