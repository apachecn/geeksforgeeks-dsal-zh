...
**在没有附加数据结构的情况下有效**：在左子树中递归查找没有孙子代且左子代和右子代的节点。 然后将`node->right`存储在`temp`中，并使`node->right = node->left`。 通过`node = node->right`，在`node`右侧的第一个节点`NULL`中插入`temp`。 重复直到将其转换为链表。
