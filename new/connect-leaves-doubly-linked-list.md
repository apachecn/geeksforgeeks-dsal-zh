...
我们需要遍历所有叶子，并通过更改它们的左右指针来连接它们。 我们还需要通过更改父节点中的左或右指针将它们从二叉树中删除。 有很多方法可以解决此问题。 在以下实现中，我们在当前链表的开头添加叶子，并使用指向 head 的指针更新列表的 head。 由于我们在开始时插入，因此我们需要以相反的顺序处理叶子。 对于逆序，我们先遍历右侧子树，然后遍历左侧子树。 我们使用返回值来更新父节点中的左或右指针。
