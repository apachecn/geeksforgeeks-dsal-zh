...
2.  将`right`递增，直到`[left = 0, right]`范围内的不同元素的数量等于`k`（或窗口大小不等于 k），让`right`是`r1`。 现在，由于子数组`[left = 0, r1]`具有`k`个不同的元素，因此所有从`Left = 0`开始，在`r1`结束的子数组也将具有`k`不同的元素。 因此，将`n - r1 + 1`添加到答案中，因为`[left, r1]`，`[left, r1 + 1]`，`[left, r1 + 2]`...`[left, n - 1]`包含所有不同的数字。

3.  现在保持`r1`不变，递增`left`。 降低前一个元素的频率，即`arr[0]`，如果其频率变为 0，则减小窗口大小。 现在，子数组为`[left = 1, right = r1]`。
