# 对数组中的三元组进行计数，使得`i < j < k`和`a[j] – a[i] = a[k] – a[j] = D`
...
给定一个数组`arr`和一个整数`D`，任务是对数组中的三元组`(arr[i], arr[j], arr[k])`进行计数 ，满足：
...
**朴素的方法**：最简单的方法是使用三个嵌套的`for`循环并查找满足给定条件的三元组。 下面是此方法的实现。
