...
> 原文： [https://www.geeksforgeeks.org/printing-paths-dijkstras-shortest-path-algorithm/](https://www.geeksforgeeks.org/printing-paths-dijkstras-shortest-path-algorithm/)
...
这个想法是创建一个单独的数组 parent []。 顶点 v 的 parent [v]的值将 v 的父顶点存储在最短路径树中。 根（或源顶点）的父级为-1。 每当我们找到通过顶点 u 的较短路径时，就会使 u 成为当前顶点的父级。

一旦构造了父数组，就可以使用以下递归函数打印路径。
