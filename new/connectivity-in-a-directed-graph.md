...
更好的主意可以是 **[强连通组件（SCC）](https://www.geeksforgeeks.org/strongly-connected-components/)算法**。 我们可以找到`O(V + E)`时间中的所有 SCC。 如果 SCC 的数量为 1，则表示图形连接牢固。 因为找到所有 SCC，所以 SCC 的算法会做额外的工作。
