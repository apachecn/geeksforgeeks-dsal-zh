...
*   如果在第 i <sup>个</sup>和第 j <sup>个</sup>索引处的元音频率超过 1，则减少该元音的数量，并移位 **i** 和 **j** 在右边和左边分别到包含元音的下一个索引。

*   如果在 **i <sup>th</sup>** 或 **j <sup>th</sup>** 索引处的元音频率等于 1，则设置 **flag1** 或 **flag2** 并继续。
...
*   遍历字符串并执行以下操作：
    ...
    *   根据**开始**处的字符不是元音或它是频率大于 1 的元音（这意味着它以前已经出现过）的条件，尽可能向右移动起点。

    *   一旦将起点尽可能地移开，请检查在 start 和当前索引之间的子字符串中是否存在所有元音。
