...
给定一个链表，重新排列它，以便转换后的链表的形式应为`a < b > c < d > e`
...
**一种简单的方法**就是通过合并排序对[排序链表](https://www.geeksforgeeks.org/merge-sort-for-linked-list/)，然后交替交换，但这需要`O(n Log n)`时间复杂度。 这里`n`是链表中元素的数量。
