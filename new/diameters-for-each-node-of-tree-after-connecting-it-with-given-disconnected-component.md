...
给定[**树**](https://www.geeksforgeeks.org/generic-treesn-array-trees/)，该树具有通过`N-1`边连接的`N`个节点和一个**非连通节点**，任务是在将给定树与给定的非连通组件连接后找到给定树的每个节点的直径。

**示例**：

> **输入**：
> 
> ![](img/b14dea91f6457dcf312aca6bfb08f428.png)
> 
> **输出**：`3 3 4 4 4 4`
> **说明**：
> 树木的初始直径为`3`。
...
> 
> ![](img/9844576c4e353fc86347dc77aed1e539.png)
> 
> **输入**：
> 
> ![](img/3947e7049763cef82f99b1ce73bb41f5.png)
> 
> **输出**：`3 2 3`
> **说明**：
> 树的初始直径为`2`
> 如果在节点`1`和节点`4`之间添加了一条边，则树的直径等于`3`（`4 -> 1 -> 2 -> 3`）。
> 如果在节点`2`和节点`4`之间添加了一条边，则树的直径等于`2`（`4 -> 2 -> 3`）。
> 如果在节点`3`和节点`4`之间添加了一条边，则树的直径等于`3`（`4 -> 3 -> 2 -> 1`）。

**方法**：要解决此问题，需要注意以下几点：

*   当断开的节点连接到形成直径末端的边时，直径增加 1。
