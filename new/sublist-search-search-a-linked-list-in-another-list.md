...
时间复杂度：O（m * n），其中m是第二个列表中的节点数，而n是第一个列表中的节点数。
**优化：**
上面的代码可以通过使用额外的空间进行优化，即将列表存储在两个字符串中并应用 [KMP算法](https://www.geeksforgeeks.org/searching-for-patterns-set-2-kmp-algorithm/)。 请参阅 [https://ide.geeksforgeeks.org/3fXUaV](https://ide.geeksforgeeks.org/3fXUaV) 以获取 [**Nishant Singh**](https://practice.geeksforgeeks.org/user-profile.php?user=_code) 提供的实施方案。
