...
[**Tarjan 算法**](https://www.geeksforgeeks.org/tarjan-algorithm-find-strongly-connected-components/)：Tarjan 算法是一种有效的图形算法，可用于查找[**强连通组件**（**SCC**）](https://www.geeksforgeeks.org/strongly-connected-components/)，通过在线性时间复杂度中仅使用有向图中的一个[DFS 遍历](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)。

**工作**：

*   在节点上执行 DFS 遍历，以便在遇到强连通组件的子树时将其删除。
...
[**Kosaraju 的算法**](https://www.geeksforgeeks.org/strongly-connected-components/)：Kosaraju 的算法也是基于[深度优先搜索](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)算法，用于在线性时间复杂度的有向图中找到 **SCC**。 该算法的基本概念是，如果我们能够最初从顶点`u`开始到达顶点`v`，那么我们应该能够从顶点`v`开始到达顶点`u`，如果是这种情况，我们可以说并得出结论，`u`和`v`的顶点是紧密相连的 ，它们位于紧密相连的子图中。
...
Tarjan 算法和 Kosaraju 算法的时间复杂度为`O(V + E)`，其中`V`表示一组顶点，`E`表示图形的边集。 Tarjan 的算法的常数因子比 Kosaraju 的算法低得多。 在 Kosaraju 的算法中，图形的[遍历至少 2 次](https://www.geeksforgeeks.org/algorithms-gq/graph-traversals-gq/)，因此常数因子可以是两倍。 在执行第二个 DFS 时，我们可以使用 Kosaraju 的算法打印正在进行的 **SCC**。 在执行 Tarjan 算法时，找到 **SCC** 子树的头后，需要花费更多时间来打印 **SCC**。

**总结**：

两种方法具有相同的线性时间复杂度，但是 **SCC** 计算的技术或过程完全不同。 Tarjan 的方法仅取决于 **DFS** 中的节点记录来对图进行分区，而 Kosaraju 的方法在图上执行两个 DFS（如果要保持原始图不变，则执行 3 个 DFS），并且非常相似 查找图的拓扑排序的方法。
