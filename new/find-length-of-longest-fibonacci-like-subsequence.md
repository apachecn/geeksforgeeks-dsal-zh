...
给定**严格增加的**正整数数组`A`，其中，`1 <= A[i] <= 10^18`。

任务是找到`A`的最长**斐波那契式子序列**的长度。 如果这样的子序列不存在，则**返回 0**。
...
**朴素的方法**：**斐波那契式序列**使得每个序列都有两个相邻的项来确定下一个预期项。

> 例如，对于 1、1，我们希望序列必须继续 2、3、5、8、13 等。

*   使用**集合**或**映射**可以快速确定数组`A`中是否存在**斐波那契数列**的下一项。 由于这些项的**指数增长**，因此每次迭代都只能进行`log(M)`搜索以获取下一个元素。
