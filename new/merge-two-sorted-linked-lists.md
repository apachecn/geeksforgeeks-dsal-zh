...
此解决方案在结构上与上述非常相似，但避免使用虚拟节点。 相反，它维护一个 struct node **指针 lastPtrRef，该指针始终指向结果列表的最后一个指针。 这解决了虚拟节点所做的相同情况—在结果列表为空时处理结果列表。 如果您试图在列表尾部建立列表，则可以使用虚拟节点或结构节点**“引用”策略（有关详细信息，请参见第 1 节）。
