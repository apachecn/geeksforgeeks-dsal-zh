...
给定一个数组[rrG0] N 个元素，则任务是查找给定数组中包含至少一个重复元素的最小子数组的长度。 子数组由数组的连续元素生成。 如果不存在这样的数组，则打印`-1`。
...
*   **简单解决方案**是使用两个嵌套循环查找每对元素。如果两个元素相等，则更新到目前为止获得的最大长度。
...
使用**散列**技术的思想，可以在`O(n)`时间和`O(n)`辅助空间中解决此问题。 想法是以线性方式遍历数组的每个元素，对于每个元素，使用哈希映射找到其最后一次出现，然后使用最后一次出现与当前索引之间的差来更新最小长度的值。 同样，用当前索引的值更新元素最后一次出现的值。
