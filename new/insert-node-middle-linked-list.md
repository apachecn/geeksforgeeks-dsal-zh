...
使用一个遍历查找节点数或链接的长度。 设为`len`。 如果`len`是偶数，则计算`c = len / 2`，否则，则计算`c = (len + 1) / 2`。 再次遍历前`c`个节点，并在第`c`个节点之后插入新节点。
...
基于乌龟和野兔算法，该算法使用两个指针，一个称为**慢速**，另一个称为**快速**。 该算法有助于找到链表的中间节点。 在[此帖子](https://www.geeksforgeeks.org/merge-sort-for-linked-list/)中对此进行了说明。 现在，您可以在通过上述过程获得的中间节点之后插入新节点。 这种方法只需要遍历列表。
