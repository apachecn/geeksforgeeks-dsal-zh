...
这样形成的结构是循环单链表，如下所示：](http://quiz.geeksforgeeks.org/circular-linked-list/)
...
要实现循环单链列表，我们采用指向列表最后一个节点的外部指针。 如果我们的指针最后指向最后一个节点，则`last -> next`将指向第一个节点。

![](img/1fbce670d977b177536b1954e5a234d2.png)

指针`last`指向节点`Z`，而指针的下一个指向节点`P`。

**为什么我们采用了指向最后一个节点而不是第一个节点的指针？**

为了在开始时插入节点，我们需要遍历整个列表。 同样，要插入到末尾，必须遍历整个列表。 如果我们使用指向最后一个节点的指针，则在两种情况下都不需要遍历整个列表。 因此，无论列表的长度如何，插入开头或结尾处都会花费固定的时间。
