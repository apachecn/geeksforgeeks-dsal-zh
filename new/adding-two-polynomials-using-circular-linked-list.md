...
2.  遍历两个多项式并检查以下条件：
    ...
    *   如果第一个多项式的 **x** 的幂等于第二个多项式的 **x** 的幂，并且 **y 的幂**大于 **y 的幂。 然后，第二个多项式的**将第一个多项式的节点存储在结果多项式中，并增加多项式 **1** 的计数器。
    *   如果第一个多项式的 **x** 的幂等于第二个多项式的 **x** 的幂，并且 **y 的幂等于第一个多项式的**的幂等于 **y 的幂 然后，第二个多项式的**将两个多项式的系数之和存储在结果多项式中，并同时增加多项式 **1** 和多项式 **2** 的计数。
