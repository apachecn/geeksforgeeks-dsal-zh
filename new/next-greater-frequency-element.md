...
> ……。 **a）**将当前元素的位置标记为“ i”。
> ……。 **b）**如果堆栈顶部指向的元素的频率比当前元素的频率高，则将当前位置 i 推入堆栈
> …… 。 **c）**如果堆栈顶部指向的元素的频率比当前元素的频率小，并且堆栈不为空，则请执行以下步骤：
> ……。 ***i）*** 继续弹出堆栈
> ……。 ***ii）*** 如果步骤 c 中的条件失败，则将当前位置 i 推入堆栈。
> 4）步骤 3 中的循环结束后，从堆栈中弹出所有元素 并将-1 打印为不存在的下一个更高频率元素。
