...
我们可以使用哈希和堆栈数据结构来有效解决许多情况。 一种简单的哈希技术是将值用作索引，将每个元素的频率用作值。 我们使用堆栈数据结构来存储元素在数组中的位置。

1.  创建一个列表，使用值作为索引来存储每个元素的频率。
1.  将第一个元素的位置压入堆栈。
1.  逐个拾取其余元素的位置，然后循环执行以下步骤。
    1.  将当前元素的位置标记为`i`。
    1.  如果堆栈顶部指向的元素的频率比当前元素的频率高，则将当前位置`i`推入堆栈
    1.  如果堆栈顶部指向的元素的频率比当前元素的频率小，并且堆栈不为空，则请执行以下步骤：
        1.  继续弹出堆栈
        1.  如果步骤`c`中的条件失败，则将当前位置`i`推入堆栈。
1.  步骤 3 中的循环结束后，从堆栈中弹出所有元素 并将 -1 打印，因为不存在下一个更高频率元素。
