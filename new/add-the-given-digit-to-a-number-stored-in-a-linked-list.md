...
**方法**：我们已经在[这个文章](https://www.geeksforgeeks.org/add-1-number-represented-linked-list/)讨论了将`1`添加到链表存储的数字中的方法，但是代码需要反转链表。

在本文中，我们将问题扩展到将任何数字添加到链表存储的数字中，并实现相同事情而无需反转或递归。

想法是遍历列表，并且遍历时保持指向最后一个节点的指针，该节点的值小于 9。这是因为我们要向链表中存储的数字添加一个数字。 因此，进位的最大值（如果存在）可以为`1`。 假设我们开始从最低有效位向最高有效位传播进位，那么一旦发现小于 9 的数字，传播就会停止。
