...
给定一个链表，除了下一个指针外，每个节点还具有一个子指针，该子指针可以指向也可以不指向单独的列表。 这些子列表可能具有自己的一个或多个子列表，依此类推，以产生一个多级数据结构，如下图所示。 您将获得列表第一级的头部。 展平列表，以便所有节点都出现在单级链表中。 您需要以第一层的所有节点都应排在第一位，然后是第二层的节点在前的方式来整理列表。
