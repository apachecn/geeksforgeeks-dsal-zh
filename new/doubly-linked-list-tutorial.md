...
1.  **在 DLL 的开头添加一个节点**：新节点始终添加在给定链表的开头之前。 并且，新添加的节点成为 DLL &的新头，并维护一个全局变量，用于计算当时的节点总数。

2.  遍历双链表

3.  **插入节点**：这可以通过三种方式完成：

    *   **在开头**：新创建的节点插入到头节点之前，并且头指向新节点。
...
4.  **删除节点**：这可以通过三种方式完成：
    ...
    *   **在末尾**：将尾部移到上一个节点以删除末尾的节点，并使尾部节点的下一个指针为`NULL`。

    *   **在给定位置**：假设位置`pos`上的节点的上一个节点为`X`，下一个节点为`Y`，然后执行以下操作：
...
> 链表：`2 4 9 1 21 22`
> 
> 在开头插入 1 之后的链表：`1 2 4 9 1 21 22`
>
> 在结尾插入 0 之后的链表：`1 2 4 9 1 21 22 0`
>
> 在第 3 个节点之后插入 44 之后的链表：`1 2 4 44 9 1 21 22 0`
> 
> 在开始处删除节点后的链表：`2 4 44 9 1 21 22 0`
>
> 在结束处删除节点后的链表：`2 4 44 9 1 21 22`
>
> 删除在位置 5 的节点之后的链表：`2 4 44 9 21 22`
