...
> 说明：
>
> 有三种不同的资源（1、2 和 3），其中每个两种。 最佳分配：进程`P1`有资源`[1, 2, 3]`，进程`P2`也有资源`[1, 2, 3]`。 进程`P2`具有 3 个不同的资源。
> 
> 输入：`arr[] = [1, 1, 2, 1, 3, 4]`
>
> 输出：3
...
> 说明：
>
> 存在三种不同类型的资源（1、2、3、4），3 个实例 1 和资源 2、3、4 的单个单个实例。最优分配：进程`P1`具有资源`[1, 1, 1]`，进程`P2`具有资源`[2, 3, 4]`。
>
> 进程`p2`具有 3 个不同的资源。
