...
4.  初始化变量 **res** 和 **sum** ，以分别存储分割字符串的方式数目和 **0** s 的累积和。
...
为了将输入字符串分成三部分，只需要两个剪切，这将给出三个子字符串-S1，S2 和 S3。 每个子字符串将具有相等的 0，即（总数 0）/ 3。 现在，从字符串开头开始跟踪 0 的计数。 一旦计数等于（总数 0）/ 3，就进行第一次切割。 同样，一旦计数为 0 等于 2​​ *（总数 1's）/ 3，就进行第二次切割。

**算法**

1.  计算 0 的数量。 如果不能被 3 整除，则答案= 0。
2.  如果计数为 0，则每个子字符串将具有相等的数字“ 0”，即零数字“ 0”。 因此，分割给定字符串的方法总数为选择 2 个位置以从 **n-1** 个位置中分割字符串的组合数量。 对于第一个选择，我们有 n-1 个选择，对于第二个选择，我们有 n-2 个选择。 因此，组合的总数为（n-1）*（n-2）。 由于选择相同，因此将其除以 2 的阶乘。因此答案= **（（n-1）*（n-2））/ 2。**
3.  从头开始遍历给定的字符串，并再次保持计数“ 0”，如果计数达到（“ 0”总数）/ 3，我们将开始累积第一次切割的方式； 当计数达到 2 *（总数为“ 0s”）/ 3 时，我们开始累积第二次切割的方式数。
