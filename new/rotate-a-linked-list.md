...
从头开始遍历列表，并在第`k`个节点处停止。 存储指向第`k`个节点的指针。 接下来我们可以使用第`k`个节点获得第`k + 1`个节点。 继续遍历最后一点，并存储指向最后一个节点的指针。 最后，如上所述更改指针。
...
要按`k`旋转链表，我们可以先使链表呈圆形，然后从头节点向前移动`k-1`步，使其为空，并使第`k`个节点为头部。
