...
1.  **AbstractList：**此类用于实现不可修改的列表，为此，只需扩展此AbstractList类并仅实现get（）和size（）方法。
...
| **[peekLast（）](https://www.geeksforgeeks.org/java-util-linkedlist-peek-peekfirst-peeklast-java/)** | 此方法检索但不删除此列表的最后一个元素，如果此列表为空，则返回null。 |
