...
*   对于具有最大频率的元素，有两种情况：最大频率元素将进入第一个数组，然后数组的大小分别为 **diff1-1 –** 和 **max1** 。 否则，最大频率的至少一个元素进入第二阵列，并且大小最大为 **diff1** 和 **max1** ？ **1** 对应。 然后找到数组可拆分为的最大大小为 **max（min（diff1？1，max1），min（diff1，max1？1））**。
