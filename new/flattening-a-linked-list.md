...
1.  指向主列表中下一个节点的指针（在下面的代码中我们称之为“正确”指针）

2.  指向此节点为头的链表的指针（在下面的代码中我们称之为“向下”指针）。
...
编写一个函数`flatten()`以将列表展平为单个链表。 展平的链表也应排序。 例如，对于上面的输入列表，输出列表应为`5 -> 7 -> 8 -> 10 -> 19 -> 20 -> 22 -> 28 -> 30 -> 35 -> 40 -> 45 -> 50`。

这个想法是对链表使用[合并排序的`Merge()`过程](https://www.geeksforgeeks.org/merge-sort-for-linked-list/)。我们使用`merge()`逐一合并列表。 我们以递归方式将当前列表与已经变平的列表合并。
