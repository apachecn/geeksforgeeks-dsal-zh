...
[对 0、1、2 和 2 的链表排序](https://www.geeksforgeeks.org/sort-a-linked-list-of-0s-1s-or-2s/)
...
**方法**：迭代链表。 维护 3 个名为零的指针，一个和两个指针分别指向链表的当前结尾节点，该链表分别包含 0、1 和 2。 对于每个遍历的节点，我们将其附加到其对应列表的末尾。 最后，我们链接所有三个列表。 为了避免许多空检查，我们使用三个伪指针`zeroD`，`oneD`和`twoD`作为三个列表的伪头。
