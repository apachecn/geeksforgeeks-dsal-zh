...
**方法**：此帖子中的[中讨论了上述问题的先前实现。 但是，一种实现方式要求反向链表，而另一种则使用递归。 此处讨论了`O(1)`空间复杂度解决方案，该解决方案不需要反向链表。

这个问题的主要重点是数字 9，它会创建所有更改，否则对于其他数字，我们只需要将其值增加 1，但是如果我们将节点的值更改为 9，则产生一个进位，[然后必须进行沿着链表将其传递](https://www.geeksforgeeks.org/add-1-number-represented-linked-list/)。

在链表中找到不等于 9 的最后一个节点。现在有三种情况：
