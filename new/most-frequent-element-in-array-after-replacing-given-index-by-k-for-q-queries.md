...
给定大小为`N`的数组`arr[]`，在`{i, k}`的形式查询`Q`中用`k`替换`arr[i]`之后，打印数组中的**最频繁元素**。
...
>
> 第二个查询：设置`arr [4] = 2`，会修改`arr [] = {3, 2, 2, 3, 2}`。 因此，2 具有最大频率。
>
> 第三个查询：设置`arr [0] = 4`，会修改`arr [] = {4, 2, 2, 3, 2}`。 所以 2 有最大频率。
> 
...
1.  初始化[映射](http://www.geeksforgeeks.org/map-associative-containers-the-c-standard-template-library-stl/)以存储所有数组元素的频率，并初始化[偶对](https://www.geeksforgeeks.org/set-in-cpp-stl/)的集合以存储频率元素偶对。 在集合中，将频率存储为负数。 这确保了存储在集合开头的第一对，即[`s.begin()`](https://www.geeksforgeeks.org/setbegin-setend-c-stl/)是`{负的最大频率, 最频繁元素}`偶对。

2.  对于每个查询，在删除第`i`个索引处的数组元素的同时，执行以下任务：

    *   从图中找到`arr[i]`的频率，即`mp[arr[i]]`。

    *   从集合中删除对`{-mp[arr[i]], arr[i]}`。

    *   通过将`{-mp[arr[i] – 1], arr[i]}`插入集合，在降低`arr[i]`的频率后，更新该集合。
    ...
    *   最后，对于每个查询，在集合的开头提取对。 集合中的**第一个元素**表示负的最大频率。 因此，第二个元素将是最常见的元素。 打印该对的第二个元素。
