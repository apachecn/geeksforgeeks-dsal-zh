...
这个想法是使用[**散列**](https://www.geeksforgeeks.org/hashing-set-1-introduction/)来在平均`O(n)`时间中解决。 我们将元素及其计数存储在哈希表中。 存储完计数后，我们再次遍历输入数组并打印计数不止一次的那些元素。 为了确保每个输出元素仅打印一次，我们在打印元素后将`count`设置为 0。
