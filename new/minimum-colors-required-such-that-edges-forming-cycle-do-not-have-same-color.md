...
给定一个带`V`顶点和`E`边且没有自环和多个边的有向图，任务是找到所需的最少颜色数目，以使相同颜色的边不 形成循环并找到每个边的颜色。
...
> 在上面给出的图中，它仅形成一个循环，即
> ，即顶点 连接 1、2、3 形成一个循环
> ，然后将连接 1- > 2 或 2- > 3 或 3- > 1 的边涂成
> ，并用不同的颜色着色，从而形成边 周期没有相同的颜色
> 
> **输入**：V = {1,2,3,4,5}，E = {（1，2），（1，3），（2，3），（2，4），（ 3，4），（4，5），（5，3）}
> **输出**：2
> 边的颜色– 1 1 1 1 1 1 1 2
> **说明 **：
> 在上面给定的图中，它仅形成一个循环，
...
**方法**：的想法是在图中找到循环，这可以借助的 [DFS 完成，其中当已经访问过的节点再次访问时， 一条新的边，然后用另一种颜色着色该边，否则，如果没有循环，则所有边只能用一种颜色着色。](https://www.geeksforgeeks.org/depth-first-search-or-dfs-for-a-graph/)

**算法**：

*   用颜色 1 标记每个边，将每个顶点标记为未访问。
...
| 3 | 3–>1 | {1, 2} | {1: 1, 2: 1, 3: 2} | 由于 1 已被访问，边 3 的颜色更改为 2 |
