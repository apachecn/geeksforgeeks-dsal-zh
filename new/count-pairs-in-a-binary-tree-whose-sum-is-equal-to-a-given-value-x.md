...
**1）朴素的方法**：通过任何一种树遍历方法逐一获取二叉树的每个节点。 将节点`temp`，树的**根**和值`x`传递给另一个函数，例如`findPair()`。 在函数中，借助**根**指针再次遍历树。 将这些节点与`temp`逐一求和，并检查`sum == x`。 如果是这样，则递增`count`。 计算`count /= 2`，因为通过上述方法已对一对进行了两次计数。
