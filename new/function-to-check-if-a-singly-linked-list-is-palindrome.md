...
*   一个简单的解决方案是使用列表节点栈。 这主要涉及三个步骤。

*   从头到尾遍历给定列表，并将每个访问的节点压入栈。

*   再次遍历列表。 对于每个访问的节点，从栈中弹出一个节点，并将弹出的节点的数据与当前访问的节点进行比较。
...
想法是使用函数调用栈作为容器。 递归遍历直到列表末尾。 当我们从最后一个 NULL 返回时，我们将在最后一个节点。 要与列表的第一个节点进行比较的最后一个节点。
