...
**朴素方法**：生成所有可能的颜色配置。 由于每个节点都可以使用 m 种可用颜色中的任何一种进行着色，因此可能的颜色配置总数为 m ^ V。
...
这里的方法是首先用颜色 1 为每个节点从 1 到 n 着色。然后从未访问的起始节点开始行进 BFS，以一次性覆盖所有连通组件。 在 BFS 遍历期间到达每个节点时，请执行以下操作：
