...
**方法：**我们已经在此处讨论了递归解决方案[。 在这篇文章中，我们将讨论上述问题的迭代解决方案。 在遍历时，我们以一个迭代处理 2k 个节点，并使用 join 和 tail 指针跟踪给定链表中 k 个节点组的第一个和最后一个节点。 反转链表的 k 个节点后，我们将反向列表的最后一个节点（由尾部指针指向）与原始列表的第一个节点（由连接指针指向）连接。 然后，我们移动当前指针，直到跳过接下来的 k 个节点。
